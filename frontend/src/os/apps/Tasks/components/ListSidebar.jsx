      ...
      {order.map((list) => {
        ...
        return (
          <Reorder.Item
            ...
            dragListener={editingId !== list.id}
            className="group flex items-center"
          >
            {editingId === list.id ? (
              <form
                onSubmit={(event) => submitRename(event, list)}
...
export default function ListSidebar({
  ...
  return (
    <aside className="flex h-40 w-full shrink-0 flex-col border-b border-white/[0.06] bg-black/20 @lg:h-auto @lg:w-52 @lg:border-r @lg:border-b-0">
      ...
      >
        <ReorderableLists
          key={lists.map((list) => `${list.id}:${list.name}:${list.position}`).join(',')}
          lists={lists}
